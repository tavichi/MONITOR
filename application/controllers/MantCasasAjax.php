		...
		if($datasect['status']==200)
		 {
			 foreach ($datasect['data'] as $data) {
				...
				$rp.='<tr '.$color.'>
						...
						<td>'.$data['falta'].'</td>
						<td><i '.$clase.' style="color:red; text-align: right;" id="'.$data['codcasa'].'"></i>&nbsp;&nbsp;<i '.$clase2.' style="color:gray; text-align: right;" id="'.$data['codcasa'].'"></i>'.$aviso.'</td></tr>';
			}	
			    $response['status']=200;
			    $response['data']=$rp;
		...
		if( $this->session->userdata('logueado') ==FALSE ) {
		 ...
		 echo json_encode($response);
			...
			$this->load->model('MantCasas_Model','MAN',true);
			 ...
			 $dircatastro = $this->input->post('dircatastro');
			 $nocasa = $this->input->post('nocasa');
			 $sector = $this->input->post('sector');
	    	 $codigosec = $this->input->post('codigosec');
			 $insertar=$this->MAN->InsertarDatos($nocasa,$dircatastro,$sector,$codigosec);
